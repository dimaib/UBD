...
Sub Uart_com(command As String * 1)                         'процедура обработки команд UART
   ...
   Select Case Command                                      'обработчик очереди команд
      ...
      Case "f"
         ...
         Ar_incr = 1
         Rfid_ar(ar_incr) = 0
         If Len(buf_copy) > 100 Then
            For I = 1 To 199
               Tm_char = Mid(buf_copy , I , 1) : If Asc(tm_char) = 255 Then Tm_char = ""
               'Temp_long = Asc(tm_char)
               Cons = Asc(tm_char)
               Print #2 , Hex(cons) ; " ";
               Waitus 100
               '(
               Shift Temp_long , Left , Rf_it
               Rfid_ar(ar_incr) = Rfid_ar(ar_incr) Or Temp_long
               'Print #2 , Hex(rfid_ar(ar_incr)) ; "    " ; Hex(temp_long )
               'Print #2 , Hex(console) ; " " ;
               Rf_it = Rf_it + 8                            'Rf_it=24, Rf_it=16, Rf_it=8
               If Rf_it = 32 Then
                  Rf_it = 0
                  Incr Ar_incr
                  Rfid_ar(ar_incr) = 0
           ...
           '(
            ...
            If Mid(tm_str , 1 , 90) = Lookupstr(13 , Str_uart) Then       'Смена точки доступа и пароля   "SetAP"
            ...
            If Mid(tm_str , 1 , 90) = Lookupstr(14 , Str_uart) Then       'Смена ID устройства "SetID:"
